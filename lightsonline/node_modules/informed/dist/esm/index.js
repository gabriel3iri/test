import React, { useContext, useMemo, useState, useRef, useLayoutEffect, Component } from 'react';
import tty from 'tty';
...
function () {
  ...
  _createClass(ObjectMap, null, [{
    ...
    key: "pullOut",
    value: function pullOut(object, path, index) {
      // Get the path to the array
      var pathArray = toPath_1(path);
      pathArray = pathArray.slice(0, pathArray.length - 1).join();
      debug('PathArray', pathArray); // Get the array

      var arr = get_1(object, pathArray);
      debug('Array', arr); // Pull out of array
...
var debug$1 = src('informed:Controller' + '\t');
...
function (_EventEmitter) {
  ...
  function FormController() {
    ...
    _this.fields = new Map(); // Initialize the controller state
    ...
    _this.setTouched = _this.setTouched.bind(_assertThisInitialized(_assertThisInitialized(_this)));
    ...
    _this.update = _this.update.bind(_assertThisInitialized(_assertThisInitialized(_this))); // Updater will be used by fields to update and register
  ...
  _createClass(FormController, [{
    ...
    value: function getFormApi() {
      ...
      var setValue = function setValue(field, value) {
        return _this2.fields.get(field).fieldApi.setValue(value);
      };
    ...
    value: function setValue(field, value) {
      ...
      debug$1("Setting ".concat(field, " to ").concat(value)); // Set the new value
    ...
    value: function notify(field) {
      ...
      if (notifier.notify) {
        notifier.notify.forEach(function (fieldName) {
          ...
          if (toNotify) {
            debug$1('Notifying', toNotify.field);
    ...
    value: function reset() {
      ...
      debug$1('Resetting'); // So we because all fields controll themselves and, "inform", this controller
    ...
    value: function setValues(values) {
      ...
      debug$1('Setting values'); // So we because all fields controll themselves and, "inform", this controller
      // of their changes, we need to literally itterate through all registered fields
    ...
    value: function register(field, fieldState, fieldStuff) {
      debug$1('Register', field, fieldState); // Always register the field
...
function (_React$Component) {
  ...
  function Form(props) {
    ...
    _this.controller = new FormController({
      dontPreventDefault: props.dontPreventDefault,
      initialValues: props.initialValues,
      validate: props.validate,
      validateFields: props.validateFields
    });
    _this.formApi = _this.controller.getFormApi();

    _this.controller.on('change', function () {
      return _this.forceUpdate();
    });

    _this.controller.on('change', function () {
      return _this.props.onChange && _this.props.onChange(_this.controller.getFormState());
    });

    _this.controller.on('submit', function () {
      return _this.props.onSubmit && _this.props.onSubmit(_this.controller.getFormState().values);
    });

    _this.controller.on('value', function () {
      return _this.props.onValueChange && _this.props.onValueChange(_this.controller.getFormState().values);
    });

    _this.controller.on('failure', function () {
      return _this.props.onSubmitFailure && _this.props.onSubmitFailure(_this.controller.getFormState().errors);
    });

    if (_this.props.getApi) {
      _this.props.getApi(_this.controller.getFormApi());
    }
  ...
  _createClass(Form, [{
    ...
    value: function render() {
      ...
      var _this$props = this.props,
          ...
          dontPreventDefault = _this$props.dontPreventDefault,
          rest = _objectWithoutProperties(_this$props, ["children", "getApi", "onChange", "onSubmit", "onValueChange", "initialValues", "onSubmitFailure", "render", "validate", "validateFields", "component", "dontPreventDefault"]);

      var formState = this.controller.getFormState();
      /* --- Create Provider and render Content --- */

      return React.createElement(FormRegisterContext.Provider, {
        value: this.controller.updater
...
var ArrayField = function ArrayField(_ref) {
  ...
  var formApi = useFormApi();
  var initialValues = formApi.getInitialValue(field) || initialValue; // TODO throw error if initial value and its not array

  var initialKeys = initialValues ? initialValues.map(function () {
    return uuidv4();
  }) : [];

  var _useState = useState(initialKeys),
...
var logger = src('informed:useField' + '\t'); // TODO figure out if this is bad? 
...
function useField(field) {
  ...
  var validate = fieldProps.validate,
      ...
      type = fieldProps.type; // Initialize state 

  var _useStateWithGetter = useStateWithGetter(initialValue != null ? initialValue : undefined),
  ...
  var _useState3 = useState(validateOnMount ? validate(initialValue) : undefined),
  ...
  var _useStateWithGetter3 = useStateWithGetter(0),
      ...
      cursor = _useStateWithGetter4[0],
      setCursor = _useStateWithGetter4[1],
      getCursor = _useStateWithGetter4[2];

  var _useState7 = useState(value),
      _useState8 = _slicedToArray(_useState7, 2),
      maskedValue = _useState8[0],
      setMaskedValue = _useState8[1]; // Grab the form register context
  ...
  var reset = function reset() {
    ...
    setValue(initialValue != null ? initialValue : undefined); // Setting somthing to undefined will remove it 
  ...
  var fieldValidate = function fieldValidate(val) {
    if (validate) {
      logger("Field validating ".concat(field, " ").concat(val));
      setError(validate(val, formApi.getValues()));
    }
  };
  /* ----------------- Field Api && State ----------------- */
  ...
  var ref = useRef(null); // We want to register and deregister this field when field name changes
  ...
  var shouldUpdate = [].concat(_toConsumableArray(Object.values(fieldState)), _toConsumableArray(Object.values(fieldProps)), [field]);

  var purify = function purify(children) {
    var userprops = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    return useMemo(function () {
      return children;
    }, [].concat(_toConsumableArray(shouldUpdate), _toConsumableArray(userprops)));
  };
...
var asField = function asField(Component$$1) {
  return function (props) {
    var field = props.field,
        ...
        maskOnBlur = props.maskOnBlur,
        onValueChange = props.onValueChange,
        notify = props.notify,
        keepState = props.keepState,
        maintainCursor = props.maintainCursor,
        debug = props.debug,
        type = props.type,
        mask = props.mask,
        format = props.format,
        parse = props.parse,
        rest = _objectWithoutProperties(props, ["field", "validate", "initialValue", "validateOnChange", "validateOnBlur", "validateOnMount", "maskOnBlur", "onValueChange", "notify", "keepState", "maintainCursor", "debug", "type", "mask", "format", "parse"]);
    ...
    var _useField = useField(field, fieldProps),
        fieldState = _useField.fieldState,
        fieldApi = _useField.fieldApi,
        purify = _useField.purify,
        ref = _useField.ref;

    return purify(React.createElement(Component$$1, _extends({
      fieldApi: fieldApi,
      fieldState: fieldState,
      field: field,
      forwardedRef: ref,
...
var logger$1 = src('informed:Text' + '\t');
